...
$stmt = db()->prepare(
    "SELECT messages.*, users.full_name AS sender_name, users.photo_path AS sender_photo
...
<div class="card">
    ...
    <div class="flex items-start gap-3" style="padding:1rem 0;border-bottom:1px solid #e5e7eb;">
        <?= avatarHtml($root['sender_photo'], $root['sender_name']) ?>
        <div>
    ...
    <?php foreach ($replies as $r): ?>
        <div class="flex items-start gap-3" style="padding:1rem 0;border-bottom:1px solid #e5e7eb;">
            <?= avatarHtml($r['sender_photo'], $r['sender_name']) ?>
            <div>
